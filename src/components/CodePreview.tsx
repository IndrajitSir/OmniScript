...
import { countScriptLines, scriptByteSize, tokenizeBashLine, type BashTokenType } from '../utils/highlight';
...
export function CodePreview() {
  const { script, fileName, settings, activeTemplate, activeModules, theme } = useComposer();
  const [tab, setTab] = useState<Tab>('source');
  const [copyState, setCopyState] = useState<CopyState>('idle');
  const [downloaded, setDownloaded] = useState(false);

  const lines = useMemo(() => script.replace(/\n$/, '').split('\n'), [script]);
  const tokenized = useMemo(() => lines.map((line) => tokenizeBashLine(line)), [lines]);
  ...
  return (
        ...
        <div className="flex items-center gap-1 rounded-lg border border-[var(--os-border)] bg-[var(--os-bg)]/60 p-0.5">
          {(['source', 'output'] as const).map((value) => (
            <button
              ...
              className={`rounded-md px-2 py-1 font-mono text-[10px] tracking-wide uppercase transition ${
                tab === value
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-[var(--os-border)] bg-[var(--os-bg)]">
        <div className="flex items-center gap-2 border-b border-[var(--os-border)] bg-[var(--os-surface-alt)]/70 px-3 py-2">
          ...
          <span className="ml-2 truncate font-mono text-[11px] text-[var(--os-muted)]">
        ...
        <div className="os-scroll min-h-0 flex-1 overflow-auto p-3">
          {tab === 'source' ? (
            <pre data-testid="os-source" className="font-mono text-[11.5px] leading-[1.55]">
              <code>
                {tokenized.map((tokens, index) => (
                  <div key={index} className="flex whitespace-pre">
                    <span className="mr-3 inline-block w-9 shrink-0 text-right text-[var(--os-muted)]/60 select-none tabular-nums">
                      {index + 1}
                    </span>
            ...
            <pre className="font-mono text-[11.5px] leading-[1.55] whitespace-pre-wrap">
              <code>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          type="button"
          onClick={handleCopy}
          className="inline-flex items-center gap-2 rounded-lg border border-[var(--os-border)] bg-[var(--os-surface-alt)] px-3 py-2 font-mono text-xs font-semibold text-[var(--os-text)] transition hover:border-[var(--os-accent)] hover:text-[var(--os-accent)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--os-accent)]"
        >
          ⧉ Copy Raw Source
        </button>
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-[var(--os-accent)] bg-[var(--os-accent)]/15 px-3 py-2 font-mono text-xs font-semibold text-[var(--os-accent)] transition hover:bg-[var(--os-accent)]/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--os-accent)]"
        >
        ...
        {copyState !== 'idle' ? (
          <Pill tone={copyState === 'copied' ? 'ok' : 'err'}>
            {copyState === 'copied' ? '✔ source copied to clipboard' : '✖ clipboard blocked by the browser'}
          </Pill>
        ) : null}
        {downloaded ? <Pill tone="ok">✔ {fileName} saved — run the chmod step below</Pill> : null}
      </div>

      {/* Permission mapping */}
      <div className="rounded-lg border border-dashed border-[var(--os-border)] bg-[var(--os-bg)]/50 px-3 py-2.5">
        ...
        <ol className="mt-2 flex flex-col gap-1 font-mono text-[11px] text-[var(--os-text)]">
          {steps.map((step, index) => (
            <li key={step} className="flex items-start gap-2">
              ...
              <code className="break-all text-[var(--os-accent-alt)]">{step}</code>
            </li>
          ))}
        </ol>
        <p className="mt-2 text-[11px] leading-snug text-[var(--os-muted)]">
          Browsers cannot persist the executable bit, so the file is written with the raw UTF-8 shell
          payload and a 0644 mask. `chmod +x` (or `install -m 755`) restores the executable bits, and
          the wrapper itself re-derives its own name from{' '}
          <code className="font-mono text-[var(--os-accent-alt)]">$OMNISCRIPT_COMMAND_NAME</code> so a
          symlink keeps the trigger name intact.
        </p>
      </div>
    </Panel>
  );
